...
def load_train(filename,entity_vec,relation_vec):
	f_i = open(filename,'r')
	target=[]
	samples = []
	for cc in f_i:
...
def LR(samples_train,target_train,samples_test,target_tset):
	...
	result=''
	result+= "Acc = "+str(Acc)+'\t'+'Recall = '+str(Recall)+'\t'+'Precision = '+str(Precision)+'\t'+'F1 = '+str(F1)+'\n'
	f.write(result)
		#ratio+=0.1
	f.close()
	return 
...
if __name__ == '__main__':
	if len(sys.argv) == 0:
		...
		print("\t-testLink <link prediction test file>\n")
	...
	for i in range(len(sys.argv)):
		if sys.argv[i].startswith('-'):
			...
			elif option == 'testLink':
				test_link_file = sys.argv[i+1]
	...
	# entity_vec = read_vec("result_matrix/"+entity_file,entity_vec)
	# relation_vec = read_vec("result_matrix/"+relation_file,relation_vec)
	print("entity vec num: ",len(entity_vec))
	samples_train,target_train = load_train(train_link_file,entity_vec,relation_vec)
	samples_test,target_test = load_train(test_link_file,entity_vec,relation_vec)
	#print(type(samples))
	#print(type(target))

	LR(samples_train,target_train,samples_test,target_test)
	#print("MSE = ",MSE)
