...
char network_file[MAX_STRING];
char output_dir[MAX_STRING];
char entity2id_file[MAX_STRING];
char relation2id_file[MAX_STRING];
...
void read_id()
{
    ...
    // }
    // fin.close();
    FILE* f1 = fopen(entity2id_file,"r");
    int x;
...
void read_relation()
{
    ...
    // }
    // fin.close();
    FILE* f2 = fopen(relation2id_file,"r");
    int x;
...
void read_train()
{
    ...
    FILE* f_kb = fopen(network_file,"r");
...
void update_0(int a1,int b0,int c0,int rel_b,int rel_c,int label)
{
    ...
    for(int c=0;c<entitydim;c++) {
        ...
        a1_c=entity_context[a1][c];
        //b0_c=entity_recontext[b0][c];
        //c0_c=entity_recontext[c0][c];
        b0_c=entity_vec[b0][c];
        c0_c=entity_vec[c0][c];
    ...
    for(int i=0;i<entitydim;i++) {
        ...
        vec_error_1[i]+=g*(entity_vec[b0][i] + entity_vec[c0][i] + relation_vec[rel_b][i] + relation_vec[rel_c][i]);
        //entity_recontext[b0][i] += g * entity_vec[a1][i];
        //entity_recontext[c0][i] += g * entity_vec[a1][i];
        entity_vec[b0][i] += g * entity_context[a1][i];
        entity_vec[c0][i] += g * entity_context[a1][i];
        relation_vec[rel_b][i] += g * entity_context[a1][i];
        relation_vec[rel_c][i] += g * entity_context[a1][i];
    }
    for(int c=0;c<entitydim;c++) {
...
void update_1(int a0,int b1,int c0,int rel_b,int rel_c,int label)
{
    ...
    double y=0,g=0;
    for(int i=0;i<entitydim;i++) {
        y+=entity_context[b1][i] * (entity_vec[a0][i] + relation_vec[rel_b][i]) + entity_context[a0][i] * (entity_vec[c0][i] + relation_vec[rel_c][i]);
    }
    y=FastSigmoid(y);
    g=(label - y) * rho;
    for(int i=0;i<entitydim;i++) {
        //vec_error_0[i]+=g*(entity_context[b1][i]+entity_recontext[c0][i] + relation_vec[rel_c][i]);//entity_vec[a0][i]
        vec_error_0[i]+=g*(entity_vec[a0][i]+relation_vec[rel_b][i]);
        vec_error_1[i]+=g*(entity_vec[c0][i] + relation_vec[rel_c][i]);//entity_context[b1][i]
        entity_vec[c0][i]+=g*entity_context[a0][i];
        relation_vec[rel_b][i]+=g*entity_context[b1][i];
        relation_vec[rel_c][i]+=g*entity_context[a0][i];
        //b1=k,c0=j
    }
    for(int c=0;c<entitydim;c++) {
        entity_vec[a0][c]+=g*entity_context[b1][c];
	entity_context[b1][c]+=vec_error_0[c];
...
void train()
{
    for(int iter2=0;iter2<1000;iter2++) {
            ...
            if(x >= 2) {
                ...
                while(i < x - 1) {
                    ...
                    if (flag == 0) {
                        ...
                        normal(entity_context[it->first]);
                        normal(entity_vec[vertex_b]);
                        normal(entity_vec[vertex_c]);
        ...
        if(iter2%50 == 0 && iter2 != 0)
        {
            ...
            fclose(fo);*/
            string output_dir_str(output_dir);

            stringstream ss;
            ss << iter2;
            string striter2 = ss.str();
            write(output_dir_str+striter2+"entity_vec.txt",entity_vec);
            write(output_dir_str+striter2+"entity_context.txt",entity_context);
            write(output_dir_str+striter2+"relation_vec.txt",relation_vec);
	    //write("result1/"+striter2+"entity_recontext.txt",relation_vec);
        }
    }
}

int makefiledir(char *filedir){
    if(access(filedir, 0) == -1){
        cout<<"make dir "<<filedir<<endl;
        int flag = mkdir(filedir, 0777);
        if(flag == 0){
            cout<<"make dir "<<filedir<<" successfully"<<endl;
            return 0;
        }else{
            cout<<"make dir "<<filedir<<" errorly"<<endl;
            exit(0);
        }
    }
    return 0;
...
int main(int argc, char** argv)
{
    int i;
    if (argc == 1){
        printf("please input Parameters for training:\n");
        printf("\t-trainset <filename>\n");
        printf("\t\tUse network data from <filename> to train the model\n");
        printf("\t-outputdir <outputdir>\n");
        printf("\t\tSave the learnt embeddings in the directory <outputdir>\n");
        printf("\t-entity2id <filename>\n");
        printf("\t\tthe directory of entity name to id\n");
        printf("\t-relation2id <filename>\n");
        printf("\t\tthe directory of relation name to id\n");
        return 0;
    }
    if ((i = ArgPos((char *)"-trainset", argc, argv)) > 0) strcpy(network_file, argv[i + 1]);
    if ((i = ArgPos((char *)"-outputdir", argc, argv)) > 0) strcpy(output_dir, argv[i + 1]);
    if ((i = ArgPos((char *)"-entity2id", argc, argv)) > 0) strcpy(entity2id_file, argv[i + 1]);
    if ((i = ArgPos((char *)"-relation2id", argc, argv)) > 0) strcpy(relation2id_file, argv[i + 1]);
    if(makefiledir(output_dir)){
    ...
    train();
    string output_dir_str(output_dir);
    write(output_dir_str + "entity_vec.txt",entity_vec);
    write(output_dir_str + "entity_context.txt",entity_context);
    write(output_dir_str + "relation_vec.txt",relation_vec);
    return 0;
} 
